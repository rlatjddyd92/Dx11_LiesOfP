...
_Vec3 CModel::Play_Animation(_float fTimeDelta)
{
	...
	Update_Animation(fAddTime);

	//저 부모뼈들 이름으로 인덱스 찾아서 해당 인덱스 업데이트 멈추고 확인해서 나머지 팔은 잘 움직인다면

	//분리된 부분 업데이트
	Update_Animation_Boundary(fAddTime);
...
void CModel::Update_Animation_Boundary(_float fTimeDelta)
{
	if (m_isUseBoundary)
	{
		if (m_isChangeAni_Boundary)
		{
			...
			if (isChangeEnd)
			{
				...
				m_iCurrentFrame_Boundary = m_Animations[m_iCurrentAnimIndex_Boundary]->Update_TransformationMatrices(m_Bones, &m_CurrentTrackPosition_Boundary, m_KeyFrameIndices[m_iCurrentAnimIndex_Boundary], m_isLoop_Boundary, &m_isEnd_Animations_Boundary[m_iCurrentAnimIndex_Boundary], 0, true, &m_isBoneUpdated, true);
...
DISSOLVE_PARTICLE_DESC CModel::Create_Particles(const DISSOLVE_INSTANCE_DESC& ParticleDesc, vector<DISSOLVE_PARTICLE>& Instances)
{
