...
void CS_SPREAD_MAIN(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (0 == iIndex % iNumTailInstance)
    {
        ...
        if (0.f == fPadding)
        {
            ...
            HeadParticle.particle.vLook = HeadParticle.particle.vTranslation - vWorldPivot;
        }
        
        float4 vDir = HeadParticle.particle.vLook;
        vDir = normalize(vDir);
        ...
        if (fTime < fTimeDelta)
        {
            ...
            HeadParticle.vNextRandomPos = float4(normalize(HeadParticle.vNextRandomPos).xyz * fTailDistance, 1.f);
        }
