...
CDissolve_Fire::CDissolve_Fire(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CDissolve_Effect(pDevice, pContext)
{
}

CDissolve_Fire::CDissolve_Fire(const CDissolve_Fire& Prototype)
    : CDissolve_Effect(Prototype)
{
}

HRESULT CDissolve_Fire::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CDissolve_Fire::Initialize(void* pArg)
{
...
void CDissolve_Fire::Priority_Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;
...
void CDissolve_Fire::Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;
...
void CDissolve_Fire::Late_Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;
...
void CDissolve_Fire::Set_On(_bool bOn)
{
    if (true == bOn)
    {
        __super::Set_On(bOn);
...
HRESULT CDissolve_Fire::Ready_Components()
{
    /* FOR.Com_Texture */
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_T_SubUV_Fire_17_ECD_8x8_SC_LGS"),
...
CDissolve_Fire* CDissolve_Fire::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CDissolve_Fire* pInstance = new CDissolve_Fire(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Create Failed : CDissolve_Fire"));
        Safe_Release(pInstance);
    }
    return pInstance;
}

CDissolve_Fire* CDissolve_Fire::Clone(void* pArg)
{
    CDissolve_Fire* pInstance = new CDissolve_Fire(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX(TEXT("Clone Failed : CDissolve_Fire"));
        Safe_Release(pInstance);
    }
    return pInstance;
}

void CDissolve_Fire::Free()
{
    __super::Free();
