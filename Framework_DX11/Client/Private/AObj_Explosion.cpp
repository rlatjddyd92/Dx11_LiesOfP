    ...
    if (pOther->Get_Tag() == TEXT("Player"))
    {
        ...
        if (!bOverlapCheck)
        {
            ...
            if (bHitCheck && !pPlayer->Get_IsInvicible())
            {
                ...
                CEffect_Manager::Get_Instance()->Add_Effect_ToLayer(LEVEL_GAMEPLAY, TEXT("Player_Impact"), pOther->Get_Transform()->Get_WorldMatrix_Ptr(),
                    pPlayer->Get_Model()->Get_BoneCombindTransformationMatrix_Ptr(pPlayer->Get_Model()->Get_UFBIndices(UFB_CHEST)), _Vec3(0.f, 0.f, 0.f), vDir);
            }
        }
    }
}
