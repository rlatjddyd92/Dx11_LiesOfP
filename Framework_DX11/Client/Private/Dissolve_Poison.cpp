...
CDissolve_Poison::CDissolve_Poison(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CDissolve_Effect(pDevice, pContext)
{
}

CDissolve_Poison::CDissolve_Poison(const CDissolve_Poison& Prototype)
    : CDissolve_Effect(Prototype)
{
}

HRESULT CDissolve_Poison::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CDissolve_Poison::Initialize(void* pArg)
{
    ...
    m_iShaderIndex = 6;
    m_vTexDevide = { 8.f, 8.f };
    m_fSpriteSpeed = 1.f;
    m_vStartScaling = { 1.f, 1.f };
    m_vScalingRatio = { 3.f, 3.f };
    m_fRotationPerSecond = { 180.f };


    m_iState = CVIBuffer_Instancing::STATE_RANDOM | CVIBuffer_Instancing::STATE_LOOP | CVIBuffer_Instancing::STATE_DECEL;
    m_iGeomState = CParticle_Effect::PS_GROW;
    return S_OK;
}

void CDissolve_Poison::Priority_Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;
...
void CDissolve_Poison::Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;
...
void CDissolve_Poison::Late_Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;
...
HRESULT CDissolve_Poison::Render()
{
...
void CDissolve_Poison::Set_On(_bool bOn)
{
    if (true == bOn)
    {
        __super::Set_On(bOn);
...
HRESULT CDissolve_Poison::Ready_Components()
{
    /* FOR.Com_Texture */
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_T_SubUV_Smoke_11_8x8_SC_GDH"),
...
CDissolve_Poison* CDissolve_Poison::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CDissolve_Poison* pInstance = new CDissolve_Poison(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Create Failed : CDissolve_Poison"));
        Safe_Release(pInstance);
    }
    return pInstance;
}

CDissolve_Poison* CDissolve_Poison::Clone(void* pArg)
{
    CDissolve_Poison* pInstance = new CDissolve_Poison(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX(TEXT("Clone Failed : CDissolve_Poison"));
        Safe_Release(pInstance);
    }
    return pInstance;
}

void CDissolve_Poison::Free()
{
    __super::Free();
