...
HRESULT CDissolve_Sophia_Death::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CDissolve_Sophia_Death::Initialize(void* pArg)
{
    ...
    m_iGeomState = CParticle_Effect::PS_SHRINK;
    m_vScalingRatio = { 0.5f, 0.5f };

    return S_OK;
}

void CDissolve_Sophia_Death::Priority_Update(_float fTimeDelta)
{
    if (false == m_bOn)
        m_bOn = true;
}

void CDissolve_Sophia_Death::Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;

    m_fThreshold -= fTimeDelta * 0.25f;

    CVIBuffer_Instancing::PARTICLE_MOVEMENT Movement = {};

    Movement.iState = CVIBuffer_Instancing::STATE_RANDOM | CVIBuffer_Instancing::STATE_DECEL;
...
void CDissolve_Sophia_Death::Late_Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;
...
HRESULT CDissolve_Sophia_Death::Render()
{
...
CDissolve_Sophia_Death* CDissolve_Sophia_Death::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CDissolve_Sophia_Death* pInstance = new CDissolve_Sophia_Death(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Create Failed : CDissolve_Sophia_Death"));
...
CGameObject* CDissolve_Sophia_Death::Clone(void* pArg)
{
    CDissolve_Sophia_Death* pInstance = new CDissolve_Sophia_Death(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX(TEXT("Clone Failed : CDissolve_Sophia_Death"));
        Safe_Release(pInstance);
    }
...
void CDissolve_Sophia_Death::Free()
{
    __super::Free();
