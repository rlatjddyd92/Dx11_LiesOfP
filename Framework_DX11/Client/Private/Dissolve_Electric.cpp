...
CDissolve_Electric::CDissolve_Electric(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CDissolve_Effect(pDevice, pContext)
{
}

CDissolve_Electric::CDissolve_Electric(const CDissolve_Electric& Prototype)
    : CDissolve_Effect(Prototype)
{
}

HRESULT CDissolve_Electric::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CDissolve_Electric::Initialize(void* pArg)
{
...
void CDissolve_Electric::Priority_Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;
...
void CDissolve_Electric::Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;
...
void CDissolve_Electric::Late_Update(_float fTimeDelta)
{
    if (false == m_bOn)
        return;
...
HRESULT CDissolve_Electric::Render()
{
...
void CDissolve_Electric::Set_On(_bool bOn)
{
    if (true == bOn)
    {
        __super::Set_On(bOn);
...
HRESULT CDissolve_Electric::Ready_Components()
{
    /* FOR.Com_Texture */
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_T_Electro_02"),
...
CDissolve_Electric* CDissolve_Electric::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CDissolve_Electric* pInstance = new CDissolve_Electric(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Create Failed : CDissolve_Electric"));
        Safe_Release(pInstance);
    }
    return pInstance;
}

CDissolve_Electric* CDissolve_Electric::Clone(void* pArg)
{
    CDissolve_Electric* pInstance = new CDissolve_Electric(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX(TEXT("Clone Failed : CDissolve_Electric"));
        Safe_Release(pInstance);
    }
    return pInstance;
}

void CDissolve_Electric::Free()
{
    __super::Free();
