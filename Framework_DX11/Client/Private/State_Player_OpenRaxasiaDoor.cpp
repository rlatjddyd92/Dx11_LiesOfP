...
HRESULT CState_Player_OpenRaxasiaDoor::Initialize(_uint iStateNum, void* pArg)
{
    ...
    m_iAnimation_DoorPush = m_pPlayer->Get_Model()->Find_AnimationIndex("AS_Pino_Prop_DoubleDoor_Slide", 0.9f);
...
HRESULT CState_Player_OpenRaxasiaDoor::Start_State(void* pArg)
{
    if (m_pTowerDoor == nullptr)
        m_pTowerDoor = static_cast<CTowerDoor*>(m_pGameInstance->Find_Object(LEVEL_GAMEPLAY, TEXT("Layer_LastDoor"), 0));
    ...
    _Vec3 vInitPos = vTowerDoorPos + vTowerDoorLook * 1.1f;
