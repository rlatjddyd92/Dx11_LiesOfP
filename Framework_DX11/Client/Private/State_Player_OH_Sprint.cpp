...
CState_Player_OH_Sprint::CState_Player_OH_Sprint(CFsm* pFsm, CPlayer* pPlayer)
    :CState{ pFsm }
    , m_pPlayer{ pPlayer }
{
}

HRESULT CState_Player_OH_Sprint::Initialize(_uint iStateNum, void* pArg)
{
    m_iAnimation_Sprint = m_pPlayer->Get_Model()->Find_AnimationIndex("AS_Pino_O_Sprint", 2.5f);
    m_iAnimation_SprintEnd = m_pPlayer->Get_Model()->Find_AnimationIndex("AS_Pino_O_Sprint_Stop", 2.5f);
...
HRESULT CState_Player_OH_Sprint::Start_State(void* pArg)
{
...
void CState_Player_OH_Sprint::Update(_float fTimeDelta)
{
    ...
    m_pPlayer->Decrease_Stamina(3.f);
...
_bool CState_Player_OH_Sprint::Move(_float fTimeDelta)
{
...
{
    CState_Player_OH_Sprint* pInstance = new CState_Player_OH_Sprint(pFsm, pPlayer);

    if (FAILED(pInstance->Initialize(iStateNum, pArg)))
    {
        MSG_BOX(TEXT("Failed to Created : CState_Player_OH_Sprint"));
...
void CState_Player_OH_Sprint::Free()
{
    __super::Free();
}